...
function startViaBinary(config, model) {
  ...
  // Defaults applied unless the model overrides them (under either alias).
  const defaults = { 'ctx-size': 8192, 'n-gpu-layers': 99 };
  ...
  for (const [flag, value] of Object.entries(defaults)) {
    if (!(flag in canonical)) canonical[flag] = { key: flag, value };
  }
...
export async function ensureModelLoaded(config, modelName) {
  ...
  loadPromise = (async () => {
    ...
    proc.on('exit', (code) => {
      ...
      console.log(`llama.cpp arrete (code ${code}, modele ${wasModel})`);
